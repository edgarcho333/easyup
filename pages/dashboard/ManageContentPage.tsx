...
export const ManageContentPage: React.FC = () => {
  ...
  const handleConfirmApprove = async () => {
    ...
    try {
      ...
      if (selectedIdea.status === 'pending_final_review') {
      ...
      <div className="flex-1 pb-10 animate-in fade-in slide-in-from-bottom-4 duration-500">
        {isLoading ? (
           ...
           <>
            ...
            <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6 mb-6">
              <div className="flex items-center gap-4">
                 <div>
                    ...
                    <p className="text-slate-500 font-medium mt-1">Create, review, and schedule posts.</p>
                 </div>
              </div>
              ...
              <div className="flex flex-col sm:flex-row gap-3">
                 <div className="relative flex-1">
                    ...
                    <input 
                      className="pl-9 pr-4 py-2.5 rounded-lg border border-slate-200 text-sm focus:ring-2 focus:ring-primary-100 focus:border-primary-400 outline-none w-full sm:w-64 transition-all"
                 ...
                 <div className="flex gap-2">
                    {isSelectionMode ? (
                        <div className="flex gap-2 animate-in fade-in slide-in-from-right-4">
                            ...
                            <Button variant="ghost" onClick={toggleSelectionMode} title="Cancel Selection">
                                <X className="h-4 w-4" />
                            </Button>
                        </div>
                    ) : (
                        <>
                            <Button variant="outline" onClick={toggleSelectionMode} className="hidden sm:flex">Select</Button>
                            <Button onClick={() => setIsCreateModalOpen(true)} className="shadow-sm shadow-primary-200">
            ...
            <div className="flex flex-col sm:flex-row p-1.5 bg-slate-100 rounded-xl w-full lg:w-fit mb-6">
              <button
              ...
              >
                ...
                Concept
                <span className={`ml-1.5 text-xs px-2 py-0.5 rounded-full ${activeTab === 'concept' ? 'bg-blue-50 text-blue-600' : 'bg-slate-200 text-slate-500'}`}>
                  {conceptIdeas.length}
                </span>
              </button>
              ...
              >
                ...
                Visuals
                <span className={`ml-1.5 text-xs px-2 py-0.5 rounded-full ${activeTab === 'production' ? 'bg-purple-50 text-purple-600' : 'bg-slate-200 text-slate-500'}`}>
              ...
              >
                ...
                Ready
                <span className={`ml-1.5 text-xs px-2 py-0.5 rounded-full ${activeTab === 'ready' ? 'bg-green-50 text-green-600' : 'bg-slate-200 text-slate-500'}`}>
            ...
            {activeTab === 'ready' ? (
               ...
               <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-5 gap-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
                  ...
                  {getActiveList().length === 0 && (
                      <div className="col-span-full py-24 flex flex-col items-center justify-center text-slate-400 border-2 border-dashed border-slate-200 rounded-3xl bg-slate-50/30">
                        <div className="h-16 w-16 bg-white rounded-full shadow-sm flex items-center justify-center mb-4">
                            <LayoutGrid className="h-8 w-8 text-slate-300" />
                        </div>
                        <h3 className="text-lg font-medium text-slate-600">No content found</h3>
                        <p className="text-sm text-slate-400 mt-1">
                          {activeTab === 'concept' ? 'Start by creating a new idea draft.' : 'Items will appear here as they progress.'}
                        </p>
                        {activeTab === 'concept' && (
                            <Button onClick={() => setIsCreateModalOpen(true)} variant="outline" className="mt-4 bg-white">
                              Create First Idea
                            </Button>
                        )}
                      </div>
      ...
      {isFeedbackModalOpen && (
         <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 animate-in fade-in duration-200">
           <div className="bg-white rounded-xl shadow-2xl w-full max-w-md p-6">
             ...
             <p className="text-sm text-slate-500 mb-4">Provide feedback on what needs to be improved.</p>
             <textarea
